...
void calculateSolutions2(int writeToRow, std::vector<int>& gameBoard, int N, std::vector<std::vector<int>>& solutions)
{
    // for each column
    for (int i = 0; i < N; ++i)
    {
        ...
        if (boardIsValidSoFar(writeToRow, gameBoard))
        {
            ...
            if (nextWriteToRow != N) // haven't filled the chessboard yet, so fill the next row
                calculateSolutions2(nextWriteToRow, gameBoard, N, solutions);
...
void calculateSolutions1(std::vector<std::vector<int>>& gameBoards, int N, std::vector<std::vector<int>>& solutions)
{
    // for each column
	...
	gameBoards.Add(gameBoard);
        ...
        const auto nextWriteToRow = writeToRow + 1;
        if (nextWriteToRow != N) // haven't filled the chessboard yet, so fill the next ro
            calculateSolutions2(nextWriteToRow, gameBoards.back(), N, solutions);
        else // filled the chessboard, so save the solution
	    solutions.push_back(gameBoards.back());
        }
    }
}
...
{
    std::vector<std::vector<std::vector<int>> gridInvalidCells; 
    std::vector<std::vector<int>> solutions;
    std::vector<std::vector<int>>gameBoards;
    
    calculateSolutionsRecursive(0, gameBoards, N, solutions);
